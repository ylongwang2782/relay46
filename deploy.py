...
'''
    ...
    def _generate_nas_docker_compose(self) -> str:
        ...
        # Collect ports for services with local_backend
        extra_ports = []
        for service in services:
...
'''
    ...
    def _generate_nas_nginx_conf(self) -> str:
        ...
        # Generate location blocks for each service (for HTTPS direct access)
        location_blocks = ""
        for service in services:
...
'''
            if host_header == 'backend':
                location_blocks += f'            proxy_set_header Host {service.get("local_backend", "host.docker.internal:" + str(service["backend_port"]))};\n'
            else:
...
http {{
    ...
    # HTTPS server for IPv6 direct access
    server {{
        listen 443 ssl http2;
        listen [::]:443 ssl http2;
...
'''
    ...
    def request_certificates(self, force_renew: bool = False) -> bool:
        ...
        if removed_records:
            ...
            import time
            print("  Waiting for DNS propagation (5s)...")
            time.sleep(5)
